...
let NotifyFilesList = Backbone.View.extend({
    ...
    listNotifyFile(options = {}) {
        ...
        const isTeacherRole = global.permissions.IS_TEACHER_ROLE;
        const listFileAction = isTeacherRole ? 'listGroupMessageFile' : 'listNotifyFile';
        service[listFileAction]({
            pageSize,
            pageNo
        }, (response) => {
            if (response.rs) {
            ...
            columns: [
                {
                    ...
                    content: (item, index) => {
                        ...
                        let publishBtnText = item.succCount
                                    ? `<a class="orange" href="#groupMessage/publish/${item.fileId}">发布通知</a>`
