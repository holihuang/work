...
var Dialog = Backbone.View.extend({
        ...
        if (hasUploadPicBtn) {
            ...
            for (let i = 0, len = uploadArr.length; i < len; i++) {
                common.picUploader({
                    ...
                    if (response.rs) {
                        if (typeof uploadArr[i].callback === 'function') {
                            let {width, height} = response.resultMessage[0];
                            let flag = uploadArr[i].callback({width, height});
                            if (!flag) {
                                //如果返回错误，则返回，不再继续执行
                                $('#' + uploadArr[i].uploadPicBtnId).removeClass('disabled');
                                return;
                            }
                        }
    ...
    format(data) {
        var {type, title, content, showHeader = true, showFooter = true} = data;
        if (type === 1) {
