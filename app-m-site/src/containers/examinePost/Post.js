...
import {GET_POST_DETAIL_REQUESTED, GET_PRAISE_USER_LIST_REQUESTED, ON_RESET_POST_STORE,} from '../../constants/examinePost';
class Post extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        // 帖子isHide不为3，deleteFlag字段为1时跳转到不可用页面
        if (Object.keys(postMaster).length && (postMaster.isHide !== 3 || postMaster.deleteFlag)) {
    ...
    formatTime(time) {
        ...
        if (year == nYear && month == nMonth) {
            ...
            if (day == nDay) {
                ...
                let interval = now - new Date(time) //计算时间差
                //计算相差天数
